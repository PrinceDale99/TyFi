...
# Now replace the return statement of FarmerDashboard
# Find the correct return by searching for <div className="w-full max-w-7xl
return_idx = text.find('return (', idx_end_brace)
# Wait, ParallaxCard's return might still be before if I didn't search after FarmerDashboard declaration.
# But idx_end_brace is AFTER the FarmerDashboard declaration! So eturn_idx is the first eturn ( inside FarmerDashboard.
# Let's double check.
        ...
        <ParallaxCard className="lg:col-span-4 p-6 flex flex-col h-[600px] overflow-y-auto">
          ...
          {!isSandboxEnabled ? (
            <div className="py-6 text-center bg-emerald-500/5 border border-emerald-500/10 rounded-xl mt-8">
              ...
              <p className="text-[10px] text-slate-400 font-medium px-4">Simulator disabled. Strictly relying on decentralized Oracle feeds.</p>
            </div>
          ) : (
            <>
