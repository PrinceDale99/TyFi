...
const AssetDistribution: React.FC<AssetDistributionProps> = ({ farms }) => {
  ...
  const totalValue = farms.reduce((sum, f) => sum + (f.totalCropValue || 0), 0);
        ...
        <div className="space-y-6 flex-1 overflow-y-auto pr-1 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-white/5 rounded-xl p-3 border border-white/5">
              ...
              <div className="text-lg font-bold text-sky-400">{totalValue.toLocaleString()} XLM</div>
              <div className="text-[9px] text-slate-400 font-medium">≈ {formatPhp(totalValue)}</div>
            </div>
          </div>
              ...
              {farms.map(farm => (
                <div key={farm.id} className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 hover:border-white/10 transition-colors">
                  ...
                  <div className="text-right">
                    <div className="text-xs font-bold text-white">{(farm.totalCropValue || 0).toLocaleString()} XLM</div>
                    <div className="text-[9px] text-slate-400 font-medium">≈ {formatPhp(farm.totalCropValue || 0)}</div>
                  </div>
                </div>
              ))}
            </div>
