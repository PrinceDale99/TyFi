...
/**
 * Combines physical oracle analysis with AI-driven damage estimation
 * for a more resilient and accurate payout trigger.
 */
export const calculateCombinedDamage = (
  oracleDamage: number, // 0-100 from physical model
  aiDamage?: number,    // 0-100 from Gemini AI
  confidenceScore: number = 85
): number => {
  if (aiDamage === undefined || aiDamage === null) {
    return oracleDamage;
  }

  // Weighting: 60% Oracle (Ground Truth), 40% AI (Visual/Contextual Analysis)
  // Confidence score can further adjust the AI weight
  const aiWeight = (confidenceScore / 100) * 0.4;
  const oracleWeight = 1 - aiWeight;

  const combined = (oracleDamage * oracleWeight) + (aiDamage * aiWeight);
  
  // Round to nearest integer for contract compatibility (u32)
