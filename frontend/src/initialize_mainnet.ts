...
async function initMainnet() {
  ...
  // Convert freighter wallet public key into 32-byte hex for admin_keys
  // (In browser/node environment, we can rely on xdr parsing or direct buffers)
  // The simplest is to use xdr.ScVal.scvBytes(Buffer.from(hex, 'hex'))
  
  // Here we use the raw bytes of the ed25519 public key
  const adminAddress = Address.fromString(address);
  const rawPubkey = adminAddress.toScVal().value().ed25519();
