...
# Add ParallaxCard definition before App component
parallax_card_code = """
...
"""
content = re.sub(r'export default function App\(\) \{', parallax_card_code, content)


# Add isSandboxEnabled state
state_code = """  const [isSandboxEnabled, setIsSandboxEnabled] = useState(false);
  const [isSimulatingWeather, setIsSimulatingWeather] = useState(false);"""
content = re.sub(r'const \[isSimulatingWeather, setIsSimulatingWeather\] = useState\(false\);', state_code, content)

# Modify Sandbox Weather Simulator to include the switch and use ParallaxCard
sandbox_box_pattern = re.compile(r'\{isTestnet && \(\s*<div className="glass-panel border border-indigo-500/20 shadow-\[0_0_25px_rgba\(99,102,241,0\.05\)\] relative overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">(.*?)</div>\s*\)\}', re.DOTALL)
...
sandbox_replacement = """{isTestnet && (
                  <ParallaxCard className="border-indigo-500/20 shadow-[0_0_25px_rgba(99,102,241,0.05)] animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-black text-white text-sm uppercase tracking-wider flex-1">dY \u2122 Sandbox Simulator</h3>
                      ...
                      <button 
                        onClick={() => {
                          ...
                          if (isSandboxEnabled) {
                            handleResetWeather(); // Reset if turning off
                          }
                        }}
...
content = sandbox_box_pattern.sub(sandbox_replacement, content)

# Wrap WeatherWidget and AssetDistribution in ParallaxCard
